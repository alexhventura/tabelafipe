...
import { buildBreadcrumbHtml } from './static-brand.ts';
...
export function buildVehicleStaticMain(bundle: VehiclePageBundle): string {
  ...
  const breadcrumb = buildBreadcrumbHtml(buildVehicleBreadcrumb(bundle));
  ...
  const sharePlaceholder = `<div class="[&_span]:text-slate-400 [&_button]:border-white/20 [&_button]:text-white [&_a]:border-white/20">
  <div class="flex items-center gap-2">
    <span class="text-xs text-slate-400 font-medium mr-1">Compartilhar</span>
  ...
  <div class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-2xl p-4 sm:p-5 space-y-3">
    <div class="space-y-0.5">
      ...
      ${heroMetaLine ? `<p class="text-xs text-slate-400">${escapeHtml(heroMetaLine)}</p>` : ''}
    </div>
    <div class="space-y-1 border-t border-white/10 pt-3">
      <p class="text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Preço FIPE</p>
      <p class="text-3xl sm:text-4xl font-bold tabular-nums leading-none">${escapeHtml(formatBRL(fipe.valorAtual))}</p>
      ${variacaoHtml}
      <p class="text-xs text-slate-400">FIPE ${escapeHtml(fipe.fipeCodigo)} · ${escapeHtml(formatMesReferencia(fipe.mesReferencia))}</p>
    </div>
  ...
  <summary class="px-4 py-3 cursor-pointer font-semibold text-sm list-none flex justify-between items-center min-h-[44px]">${escapeHtml(item.pergunta)}<span class="text-slate-400 group-open:rotate-180 transition-transform" aria-hidden="true">▼</span></summary>
